...
const ContactForm = ({ language }: ContactFormProps) => {
  ...
  const translations = {
    ...
    ru: {
      ...
      description: "Есть вопрос или хотите сотрудничать? Отправьте нам сообщение, и мы ответим как можно скорее.",
      name: "Имя",
      email: "Электронная почта",
      message: "Сообщение",
      namePlaceholder: "Ваше полное имя",
      emailPlaceholder: "ваша.почта@example.com",
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.errors) {
          const backendErrors: Errors = {};
          Object.keys(data.errors).forEach(key => {
            // ترجمة أخطاء backend إذا كانت موجودة
            if (key === 'name') backendErrors[key] = `${t.name} ${t.required}`;
            else if (key === 'email') backendErrors[key] = t.emailInvalid;
            else if (key === 'message') backendErrors[key] = `${t.message} ${t.required}`;
            else backendErrors[key] = data.errors[key][0];
          });
          setErrors(backendErrors);
          throw new Error('Validation failed');
        }
      ...
      if (!errors.name && !errors.email && !errors.message) {
        setErrors({
          form: error instanceof Error ? error.message : t.error
        });
      }
    } finally {
      setLoading(false);
